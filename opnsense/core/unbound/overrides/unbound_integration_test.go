...
func TestUnboundIntegration_CreateHostOverrideAndVerifyViaOpnSenseAPI(t *testing.T) {
	...
	api := opnsense.NewOpnSenseClient(address, cfg.key, cfg.secret)
	api.SetInsecureSkipVerify(true)
	hostsAPI := GetHostsOverrideApi(api)
	...
	host := &OverridesHost{Host: OverridesHostDetails{
		...
		Domain:      domain,
		Rr:          "A",
		Description: "go-opnsense integration test",
		Server:      targetIPv4,
	}}

	created, err := hostsAPI.Create(host)
	require.NoError(t, err)
	createdUUID := strings.TrimSpace(created.Host.Uuid)
	if createdUUID == "" {
		createdUUID, err = findOverrideUUIDByHostDomain(t, api, hostname, domain)
		require.NoError(t, err)
	}

	t.Cleanup(func() {
		if createdUUID != "" {
			if delErr := hostsAPI.DeleteByID(createdUUID); delErr == nil {
	...
	verifiedUUID := waitForHostOverrideApplied(ctx, t, api, hostsAPI, hostname, domain, targetIPv4, 2*time.Minute)
	if createdUUID == "" {
		createdUUID = verifiedUUID
	}
}

func TestUnboundIntegration_CreateHostAliasAndVerifyViaOpnSenseAPI(t *testing.T) {
	if os.Getenv("OPNSENSE_E2E") != "1" {
		t.Skip("set OPNSENSE_E2E=1 to run integration test")
	}
	...
	api.SetInsecureSkipVerify(true)
	hostsAPI := GetHostsOverrideApi(api)
	aliasesAPI := GetAliasesOverrideApi(api)
	serviceAPI := unbound.New(api).Service

	unique := fmt.Sprintf("%d", time.Now().UnixNano())
	baseHostname := "test-alias-base-" + unique
	aliasHostname := "test-alias-" + unique
	domain := "xyz.de"
	baseHostFQDN := baseHostname + "." + domain
	targetIPv4 := "10.201.202.203"
	...
	host := &OverridesHost{Host: OverridesHostDetails{
		...
		Domain:      domain,
		Rr:          "A",
		Description: "go-opnsense integration test alias base host",
		Server:      targetIPv4,
	}}

	createdHost, err := hostsAPI.Create(host)
	require.NoError(t, err)
	hostUUID := strings.TrimSpace(createdHost.Host.Uuid)
	if hostUUID == "" {
		hostUUID, err = findOverrideUUIDByHostDomain(t, api, baseHostname, domain)
		require.NoError(t, err)
	}
	require.NotEmpty(t, hostUUID, "created host override uuid is required for alias test")
	...
	alias := &OverridesAlias{Alias: OverridesAliasDetails{
		...
		Domain:      domain,
		Description: "go-opnsense integration test host alias",
	}}

	createdAlias, err := aliasesAPI.Create(alias)
	require.NoError(t, err)
	aliasUUID := strings.TrimSpace(createdAlias.Alias.Uuid)
	require.NotEmpty(t, aliasUUID, "created alias uuid must not be empty")

	t.Cleanup(func() {
		if aliasUUID != "" {
			_ = aliasesAPI.DeleteByID(aliasUUID)
		}
		if hostUUID != "" {
			_ = hostsAPI.DeleteByID(hostUUID)
		}
		_, _ = serviceAPI.Reconfigure()
	})
	...
	waitForHostAliasApplied(ctx, t, api, aliasUUID, hostUUID, baseHostFQDN, aliasHostname, domain, 2*time.Minute)
}
...
func isOpnSenseReachable(address, key, secret string) bool {
	...
	api := opnsense.NewOpnSenseClient(address, key, secret)
	api.SetInsecureSkipVerify(true)
...
func deleteOverrideByHostDomainIfExists(t *testing.T, api *opnsense.OpnSenseApi, hosts OverridesHostsApi, hostname, domain string) {
	t.Helper()
...
func findOverrideUUIDByHostDomain(t *testing.T, api *opnsense.OpnSenseApi, hostname, domain string) (string, error) {
	t.Helper()
